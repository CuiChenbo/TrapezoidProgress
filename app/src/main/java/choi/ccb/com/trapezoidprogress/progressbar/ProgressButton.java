...
import choi.ccb.com.trapezoidprogress.R;


/**
 * Created by chenbocui
 */
public class ProgressButton extends AppCompatTextView {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        int bgColor = a.getColor(R.styleable.ProgressButton_progressbtn_background_color, Color.parseColor("#6699ff"));
        //初始化背景颜色数组
        initGradientColor(bgColor, bgColor);
        mBackgroundSecondColor = a.getColor(R.styleable.ProgressButton_progressbtn_background_second_color, Color.LTGRAY);
        mButtonRadius = a.getFloat(R.styleable.ProgressButton_progressbtn_radius, getMeasuredHeight() / 2);
        mAboveTextSize = a.getDimension(R.styleable.ProgressButton_progressbtn_text_size, 20);
        mTrapezoidDiffer = a.getDimension(R.styleable.ProgressButton_progressbtn_trapezoid_differ, 12);
        mTextColor = a.getColor(R.styleable.ProgressButton_progressbtn_text_color, bgColor);
        mTextCoverColor = a.getColor(R.styleable.ProgressButton_progressbtn_text_covercolor, Color.WHITE);
        boolean enableGradient = a.getBoolean(R.styleable.ProgressButton_progressbtn_enable_gradient, false);
        boolean enablePress = a.getBoolean(R.styleable.ProgressButton_progressbtn_enable_press, false);
        isTrapezoid = a.getBoolean(R.styleable.ProgressButton_progressbtn_enable_trapezoid, true);
    ...
    private void drawBackground(Canvas canvas) {
        ...
        trapezoidPath.moveTo(mTrapezoidDiffer, 0);
        trapezoidPath.lineTo(getMeasuredWidth() - mTrapezoidDiffer, 0);
        trapezoidPath.lineTo(getMeasuredWidth(), getMeasuredHeight());
        trapezoidPath.lineTo(0, getMeasuredHeight());
        trapezoidPath.close();
        ...
        switch (mState) {
            ...
            case DOWNLOADING:
                ...
                if (isTrapezoid) {
                    canvas.drawPath(trapezoidPath, mBackgroundPaint); //梯形背景
                } else {
                    canvas.drawRoundRect(mBackgroundBounds, mButtonRadius, mButtonRadius, mBackgroundPaint); //圆角矩形
                }
    ...
    private boolean isCurrentTextChange = false;

    public void setCurrentTextChange(boolean isCurrentTextChange) {
        this.isCurrentTextChange = isCurrentTextChange;
    }
     ...
     * 设置开启关闭梯形
     */
    public void setIsTrapezoid(boolean isTrapezoid) {
     ...
     * 设置梯形上下边的差距
     */
    public void setTrapezoidDiffer(float mTrapezoidDiffer) {
        this.mTrapezoidDiffer = mTrapezoidDiffer;
        invalidate();
    }
    ...
    public ProgressButton setCustomerController(ButtonController customerController) {
